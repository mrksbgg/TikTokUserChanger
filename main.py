# this is a working piece of shit
...
# Messages
startmsg = f"""
👋 Welcome to <b>TikTokUserChangerBot!</b>

Using this bot, you can change your TikTok username to anything you want! Russian username, username with emojis, etc.

This bot is <a href="https://github.com/mrksbgg/TikTokUserChanger">open-source.</a>
"""

changemsg = f"""
💊 To continue, you will need to get sessionid from your TikTok account!

Click ✅ if you have sessionid or click ❓ to find out how to obtain sessionid
"""

faqmsg = f"""
🆘 <b>FAQ</b>

<b>Q:</b> Is this bot safe?
<b>A:</b> Yes, this bot is safe. This bot is <a href="https://github.com/mrksbgg/TikTokUserChanger">open-source</a> and you can host it by yourself.
"""
...
<b>PC (Safari), iPhone:</b>
Sorry, couldn't find methods to get cookies for you.
"""

enterSIDmsg = f"""
💊 Bot will ask for your sessionid
"""

enterusernamemsg = f"""
💊 Bot will ask for new username
"""
...
async def startcmd(client, message):
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(f"⚙️ Change username", callback_data="change"),
                InlineKeyboardButton("💎 Source code", url="https://github.com/mrksbgg/TikTokUserChanger/"),
...
async def handle_callback_query(client, query):
    if query.data == "backtostart":
        ...
        inline_keyboard=[
            [
                InlineKeyboardButton(f"✅", callback_data="continue"),
                InlineKeyboardButton("❓", callback_data="obtainSID"),
                InlineKeyboardButton("⬅️ Back", callback_data="backtostart")
            ]
        ]
        )
        await query.message.edit_text(
        ...
        inline_keyboard=[
            [
                InlineKeyboardButton("⬅️ Back", callback_data="change")
            ]
        ]
        )
        await query.message.edit_text(
        ...
        sid = await client.ask(chat_id=query.from_user.id, text="Enter your sessionid")
        user = get_profile(sid.text, device_id, iid)
        if user != "None":
            await client.send_message(query.from_user.id, f"💎 Your current TikTok username: `@{user}`")
            new_username = await client.ask(chat_id=query.from_user.id, text=enterusernamemsg)
            change_username(sid.text, device_id, iid, user, new_username.text)
            await client.send_message(query.from_user.id, f"✅ Changed TikTok username to: `@{new_username.text}`")
        else:
            user_us = get_profile_us(sid.text, device_id, iid)
            if user_us != "None":
                await client.send_message(query.from_user.id, f"💎 Your current TikTok username: `@{user}`")
                new_username = await client.ask(chat_id=query.from_user.id, text=enterusernamemsg)
                change_username_us(sid.text, device_id, iid, user_us, new_username.text)
                await client.send_message(query.from_user.id, f"✅ Changed TikTok username to: `@{new_username.text}`")
            else:
                await client.send_message(query.from_user.id, "⚠️ Invalid session ID or other error.")
...
if __name__ == '__main__':
    try:
        ...
        print(f"[Error] Session is locked! ({e})")
        os.remove("tiktokUserChangerBot.session")
        print("f[Info] Restarting...")
        app.start()
        me = app.get_me()
    print(f"[Info] Bot running as {me.first_name} (@{me.username})")
    idle()
